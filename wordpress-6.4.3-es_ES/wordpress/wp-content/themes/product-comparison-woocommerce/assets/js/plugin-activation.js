jQuery(document).ready(function($) {
    'use strict';
    var this_obj = product_comparison_woocommerce_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.product-comparison-woocommerce-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message product-comparison-woocommerce-plugin-install');
                button.addClass('button-primary product-comparison-woocommerce-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.product-comparison-woocommerce-recommended-plugins .product-comparison-woocommerce-activation-note').css('display','block');
    ...
    $(document).on('click', '.product-comparison-woocommerce-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .product-comparison-woocommerce-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .product-comparison-woocommerce-recommended-plugins').css('display','none');
    });
